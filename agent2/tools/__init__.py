...
_TABLE_NOTE = {
    ...
    # `무수익여신비율`과 `고정이하여신비율`이 같은 표에 나란히 있어 모델이 섞어 쓴다.
    # 회사마다 다른 행을 골라 비교하면 서로 다른 지표를 견주게 된다.
    "asset_quality": "**NPL(부실채권) 비율 = `고정이하여신비율`입니다.** "
                     "같은 표의 `무수익여신비율`은 다른 지표이니 NPL로 답하지 마십시오 "
                     "(무수익여신은 이자수익이 끊긴 여신이고, 고정이하여신은 건전성 "
                     "분류가 '고정' 이하인 여신입니다). 회사끼리 비교할 때는 "
                     "**반드시 같은 행끼리** 견주십시오. "
                     "그리고 이 표의 지표가 **그룹 기준인지 은행 단독 기준인지 표에 적혀 있지 "
