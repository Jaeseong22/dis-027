...
#: **정보한계 고지 어휘 — 단일 진실.** `reviewsheet`·`grade`가 이걸 가져다 쓴다.
#: ★ 사전이 모듈마다 따로 있어서 **같은 실수를 세 번** 했다:
#:   거부 문장 안의 금지어를 위반으로 세어 **정답을 벌줬다.**
#:   마지막엔 이 파일이 만든 차단 메시지("…산출할 수 없고")를 `reviewsheet`가
#:   위반으로 판정했다. 사전을 한 곳에 둔다(지식은 프롬프트가 아니라 데이터로).
DENIAL = re.compile(
...
#: 역할 이탈·지시문 유출 신호. **`reviewsheet`가 이걸 가져다 쓴다** — 사전은 한 곳이다.
#: 종전에는 `reviewsheet.py`에만 있었는데 그 파일은 **채점기**이고 폐기된 270 평가셋용이라,
#: 런타임 방어에는 이 어휘가 **한 줄도 걸려 있지 않았다**(2026-08-17 실측:
#: 공격 성공 답변 5종 중 밸류에이션 1종만 막혔다).
...
#:   `audit.probe` 40문항에서 공격 6건 중 1건이 정확히 이렇게 뚫렸다(§5.3 미해결 ①).
#:
