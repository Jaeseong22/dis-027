...
#: 질문 쪽 금지 주제. 차단이 아니라 `clarify` 게이트가 **비켜설 자리**를 정하는 데만 쓴다 —
#: 금지 질의는 되묻기가 아니라 거절이 정답이고(평가지표 6) 그 거절은 아래 `guard`가 한다.
#: 맨 `매수`·`매도`는 넣지 않는다 — `자기주식 처분방법 … 시장을 통한 매도`가 공시 항목명이다.
FORBIDDEN_Q = re.compile(
    _VALUATION.pattern + r"|"
    r"주가|시세|종목\s*추천|투자\s*추천|"
    r"주식(을|를)?\s*(지금\s*)?(사|팔)|사도\s*될까|팔아야|오를까|내릴까|"
    r"전망|예측|예상")
