...
`CLAUDE.md`에 "로컬 XML엔 XBRL account_id 태그가 없다"고 적혀 있었고, 그래서 지금까지
...
라벨 파싱의 대가는 실제로 치렀다 — 2호기가 삼성전자 판매비와관리비를
...
CONCEPTS = {
    ...
    # 실측: 8개 평가대상사 **전부** 이 코드를 갖고 있고 값이 도원님 정답과 일치한다
    ...
    #   classifications"). 도원님 1차 질문셋 재무 5·7번이 이걸 묻는데 개념이 없어
...
def pick(fs, concept, scope=CONSOLIDATED, year=None):
    ...
    ★ **연도를 안 주면 최신 사업연도를 쓴다.** 한 보고서에는 당기·전기·전전기가 함께
      ...
      (2025년은 524,806,125천원이고 도원님 정답도 이 값이다). 등장 순은
