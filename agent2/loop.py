...
답변에 쓰지 않는 것:
14. **도구 이름·인자를 답변에 적지 마십시오.** 사용자는 도구를 부를 수 없습니다.
    근거 자리에는 도구가 준 `출처`(접수번호가 든 문자열)를 적습니다 — 도구 이름이
    아닙니다. (실측: `(근거: list_filings, 계수)`라고 적은 사고가 있었습니다.)
15. 근거를 못 찾았으면 "공시에서 확인되지 않습니다"로 **끝냅니다.** 사용자에게
    다른 도구를 써 보라고 권하지 마십시오 — 더 찾아야 한다면 당신이 직접 도구를
    한 번 더 부르십시오. (실측: `find_tables 혹은 find_sections 도구를 사용하여
    찾아보는 것을 권장드립니다`라고 답한 사고가 있었습니다.)
...
#: 한 답변에 붙일 출처 개수 상한.
#: 3은 2사 비교까지만 감당한다 — 4대 금융지주를 비교한 답변에서 네 번째 회사의
#: 접수번호가 잘렸다. 다중 비교는 회사 수만큼 출처가 생긴다.
#: 무한정 늘리지 않는 것은 관측이 아니라 **답변**에 붙는 줄이기 때문이다
#: (실측 비용 +39자). 정답셋 다중조회의 최대 회사 수가 4사라 한 칸 여유를 둔다.
_CITE_MAX = 5
